...
// #include "display/robot16.h"
    ...
    #define BP_HAS_OTP 0
#endif
#if defined(BP_MANUFACTURING_TEST_MODE)
...
static void test_otp_subsystem(void) {
...
static void main_system_initialization(void) {
    ...
    // CAUTION:
    //       even though initialized, no printf() will be serviced
    //       until Core1 is running its infinite loop.  Output of
    //       more data than queue can hold via printf() will result
    //       in DEADLOCK as the queue isn't going to be serviced!
    // Use RTT for early boot debugging / status messages, even
    // if printf() is enabled!
    // ALTERNATIVE: modify rx/tx functions to discard inputs if
    //              core1 isn't servicing the queues yet?
    tx_fifo_init();
    rx_fifo_init();

#if BP_HAS_OTP
    BP_DEBUG_PRINT(BP_DEBUG_LEVEL_VERBOSE, BP_DEBUG_CAT_EARLY_BOOT,
        "Init: OTP subsystem testing\n"
        );
    test_otp_subsystem();
#endif

#if 0 && defined(BP_MANUFACTURING_TEST_MODE) && BP_HAS_OTP
    BP_DEBUG_PRINT(BP_DEBUG_LEVEL_VERBOSE, BP_DEBUG_CAT_EARLY_BOOT,
