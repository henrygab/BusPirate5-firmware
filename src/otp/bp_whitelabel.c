...
bool internal_get_whitelabel_row_range(BP_OTP_ROW_RANGE *result_out) {
    ...
    if (!saferotp_read_single_value_rbit3(0x059, &current_usb_boot_flags.as_uint32)) {
    ...
    uint16_t base;
    if (!saferotp_read_single_value_ecc(0x05C, &base)) {
    ...
    if (!saferotp_read_data_ecc(base, &whitelabel, sizeof(whitelabel))) {
...
void bp_otp_apply_whitelabel_data(void) {
    ...
    for (uint16_t i = 0; i < ARRAY_SIZE(_static_portion); ++i) {
        ...
        WAIT_FOR_KEY();
        saferotp_write_single_value_ecc(row, _static_portion[i]) || DIE();
    }
    ...
    for (uint16_t i = 0; i < product_extension_rows; ++i) {
        ...
        WAIT_FOR_KEY();
        saferotp_write_single_value_ecc(row, data) || DIE();
    }
    ...
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index 0: row 0x%03x\n", base + 0x0u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x0u, 0x1209u) || DIE();  // USB VID == 0x1209
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index 1: row 0x%03x\n", base + 0x1u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x1u, 0x7332u) || DIE();  // USB PID == 0x7332
    //INT_DEBUG("Whitelabel Debug: Write WHITELABEL index 2: row 0x%03x\n", base + 0x2u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x2u, 0x....u) || DIE();  // USB BCD Device
    //INT_DEBUG("Whitelabel Debug: Write WHITELABEL index 3: row 0x%03x\n", base + 0x3u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x3u, 0x....u) || DIE();  // USB LangID for strings
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index 4: row 0x%03x\n", base + 0x4u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x4u, 0x230Au) || DIE();  // USB MANU              ten   chars @ offset 0x23
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index 5: row 0x%03x\n", base + 0x5u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x5u, tmp_p  ) || DIE();  // USB PROD              XXX   chars @ offset 0x23
    //INT_DEBUG("Whitelabel Debug: Write WHITELABEL index 6: row 0x%03x\n", base + 0x6u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x6u, 0x....u) || DIE();  // USB Serial Number
    //INT_DEBUG("Whitelabel Debug: Write WHITELABEL index 7: row 0x%03x\n", base + 0x7u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x7u, 0x....u) || DIE();  // USB config attributes & max power
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index 8: row 0x%03x\n", base + 0x8u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x8u, 0x1B08u) || DIE();  // STOR VOLUME LABEL     eight chars @ offset 0x1b
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index 9: row 0x%03x\n", base + 0x9u);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0x9u, 0x1F08u) || DIE();  // SCSI INQUIRY VENDOR   eight chars @ offset 0x1f
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index A: row 0x%03x\n", base + 0xAu);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0xAu, tmp_p  ) || DIE();  // SCSI INQUIRY PRODUCT  XXX   chars @ offset 0x23
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index C: row 0x%03x\n", base + 0xCu);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0xCu, 0x1016u) || DIE();  // redirect URL          22    chars @ offset 0x10
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index D: row 0x%03x\n", base + 0xDu);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0xDu, 0x140du) || DIE();  // redirect name         13    chars @ offset 0x14
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index E: row 0x%03x\n", base + 0xEu);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0xEu, tmp_p  ) || DIE();  // info_uf2.txt product  XXX   chars @ offset 0x23
    PRINT_DEBUG("Whitelabel Debug: Write WHITELABEL index F: row 0x%03x\n", base + 0xFu);  WAIT_FOR_KEY();  saferotp_write_single_value_ecc(base + 0xFu, 0x3417u) || DIE();  // info_uf2.txt board ID 23    chars @ offset 0x34
    ...
    saferotp_write_single_value_ecc(0x05c, base) || DIE();
    ...
    if (!saferotp_read_single_value_rbit3(0x059, &old_usb_boot_flags.as_uint32)) {
    ...
    if (!saferotp_write_single_value_rbit3(0x059, new_flags.as_uint32)) {
...
bool bp_otp_lock_whitelabel(void) {
    ...
    if (!saferotp_write_single_value_byte3x(0xF86, 0x3Fu)) {
    ...
    if (!saferotp_write_single_value_byte3x(0xF87, 0x15u)) {
