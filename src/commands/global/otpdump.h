...
static_assert(sizeof(XOTP_DIRENTRY_TYPE) <= sizeof(uint16_t), "XOTP_DIRENTRY_TYPE must fit in 16-bits");

typedef struct _XOTP_DIRECTORY_ITEM {
    XOTP_DIRENTRY_TYPE EntryType;
    uint16_t StartRow;
    uint16_t RowCount; // May be zero, indicating that the entry has no associated data.
    uint16_t CRC16;
} XOTP_DIRECTORY_ITEM;
...
// Regardless of return value, `out_bytes_valid_count` will be set to
...
// * Read of ECC data resulted in an ECC error being reported
bool xotp_get_directory_item_data(const XOTP_DIRECTORY_ITEM* direntry, size_t* out_bytes_valid_count, void* out_buffer, size_t buffer_len);
...
#define XOTP_DIRTYPE_BLANK_ENTRY           ((XOTP_DIRENTRY_TYPE){ .as_uint16 = 0x0000u }); // unprogrammed page ... no data ... but may not be the end?
#define XOTP_DIRTYPE_USB_WHITELABEL        ((XOTP_DIRENTRY_TYPE){ .as_uint16 = 0x0001u }); // ECC, structure used to whitelabel (branding) bootrom
#define XOTP_DIRTYPE_PROVENANCE_X509       ((XOTP_DIRENTRY_TYPE){ .as_uint16 = 0x0002u }); // ECC, x509 certificate showing provenance of the device
#define XOTP_DIRTYPE_DIRECTORY_SEALED      ((XOTP_DIRENTRY_TYPE){ .as_uint16 = 0xFFFFu }); // appending entries intentionally disallowed
